...
(function () {
  ...
  function fetchMatchingResourceDetails(matchingResources) {
    var promises = matchingResources.map(function (resource) {
      return fetchResourceDetail(resource.id).then(function (detail) {
        return detail && detail.media ? detail : resource;
      });
    });
  ...
  function createEditionRow(resource) {
    ...
    if (availability.state === "available") {
      ...
      var editionNextReturn = getNextReturnDate(availability.returnDates);

      status.textContent = editionNextReturn
        ? "Disponível em " + formatDate(editionNextReturn)
        : "Todos reservados";
    } else {
  ...
  function createHeadRow() {
    ...
    ["Editora / ano", "Estado", "Reservas"].forEach(
      function (label) {
  ...
  function createCard(
    ...
    if (availability.state === "available") {
      ...
      status.textContent = nextReturn
        ? "Disponível em " + formatDate(nextReturn)
        : "0 licenças disponíveis";
    } else if (availability.holds > 0) {
      status.textContent =
        availability.holds +
      ...
      status.textContent = "Título disponível no BiblioLED";
    }
