...
(function () {
  ...
  function getResourceMediumLabel(resource) {
    var media = normalizeMedia(resource);

    var types = uniqueWords(
      media
        .map(function (medium) {
    ...
    return types
      .map(function (type) {
        ...
        if (type.indexOf("epub") !== -1 || type.indexOf("ebook") !== -1) {
          return "ebook";
        }
  ...
  function createEditionRow(resource) {
    ...
    edition.textContent = getResourceEditionLabel(resource);
    row.appendChild(edition);

    var medium = document.createElement("span");
    medium.className = "rbmo-biblioled-secondary";
    medium.textContent = getResourceMediumLabel(resource) || "—";
    ...
    status.className =
      "rbmo-biblioled-status rbmo-biblioled-status--" + availability.state;

    if (availability.state === "unknown") {
      status.textContent = "Consultar";
    } else {
      status.textContent =
        availability.available + " de " + availability.total;
    }
    ...
    holds.textContent = String(availability.holds || 0);
  ...
  function createHeadRow() {
    ...
    ["Editora / ano", "Suporte", "Estado", "Reservas"].forEach(
      function (label) {
  ...
  function createCard(
    ...
    if (availability.state === "available") {
      status.textContent =
        availability.available === 1
          ...
          : availability.available + " licenças disponíveis";
    } else if (availability.state === "unavailable") {
      status.textContent = "0 licenças disponíveis";
    } else {
    ...
    if (
      ...
      if (nextReturn) {
        ...
        returnElement.className = "rbmo-biblioled-return";
        returnElement.textContent =
          "Próxima devolução prevista: " + formatDate(nextReturn);
        panel.appendChild(returnElement);
      }
    }
